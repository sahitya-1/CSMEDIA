...
def mobile(request, data=None):
    ...
    if data == None:
        ...
        mobiles = Product.objects.filter(category='M').filter(discounted_price__lt=100)
    elif data == 'above':
        mobiles = Product.objects.filter(category='M').filter(discounted_price__gt=100)
...
def login(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            messages.success(request, 'Congratulations!! Login Successfully.')
            form.save()
